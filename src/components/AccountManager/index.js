...
const AccountManager = ({agreement = false, link = true}) => {
    ...
    const renderLink = () => {
        ...
        if (window.location.pathname === '/create-acc') {
            return (
                <p className='sign-text'>
                    Already have an account? <Link to="/sign-in" className='sign-link'>Sign In</Link>.
                </p>
            )
        }
        ...
        if (window.location.pathname === '/sign-in') {
            return (
                <p className='sign-text'>
                    Don't have an account? <Link to="/create-acc" className='sign-link'>Sign Up</Link>.
                </p>
            )
        }
    }
        ...
        return fields.map((field, id) => {   
            return (
                <div className='account__inner field' key={id}>
                    ...
                    <input className='field__input' type={field.type} id={field.id} placeholder={field.placeholder} onChange={(e) => onChangeInput(e)}/>
                    {/* {renderError()} */}
                </div>
            )
        })
    ...
    return (
        <div className='account'>
            ...
            <form className='account__form' onSubmit={(e) => onFormSubmit(e)}>
                ...
                <button className='account__button button' type='submit'>{renderButtonText()}</button>
